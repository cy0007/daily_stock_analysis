...
### 🛡️ 交易理念内置
- ❌ **严禁追高** - 股价涨太快（离均线超过5%）自动标记「危险」
- ✅ **趋势交易** - 均线向上发散，走势健康才买
- 📍 **精确点位** - 买入价、止损价、目标价
- 📋 **检查清单** - 每项条件用 ✅⚠️❌ 标记
- 💬 **大白话分析** - 告别专业术语，普通人也能看懂
...
🟢 买入 | 贵州茅台(600519)
📌 股价小幅回调到5日均线附近企稳，离均线只有1.2%，是好买点
💰 买入价1800 | 止损价1750 | 目标价1900
✅均线向上 ✅没涨太多 ✅成交量配合

🟡 观望 | 宁德时代(300750)
📌 股价涨太多了，离均线7.8%超过5%警戒线，别追高
⚠️ 等股价回调到均线附近再考虑
...
- ⚙️ **系统设置** - 在网页上配置 API Keys、自选股、邮件推送、定时任务，无需手动编辑 `.env` 文件
- 📝 **配置管理** - 查看/修改自选股列表
- 🚀 **快速分析** - 页面输入股票代码，一键触发分析
- 📊 **实时进度** - 分析任务状态实时更新，支持多任务并行
- 🔐 **安全存储** - 敏感配置（API Keys、密码）加密存储到数据库
...
| 配置项 | 说明 |
|--------|------|
| **API Keys** | Gemini、OpenAI、DeepSeek、智谱AI、Tavily、SerpAPI、Tushare |
| **自选股** | 支持 A股、港股、美股代码 |
| **邮件推送** | 发件人、密码、收件人，支持测试发送 |
| **定时任务** | 启用开关、执行时间、大盘复盘开关 |
...
├── web/                 # WebUI 模块
│   ├── handlers.py      # 请求处理器
│   ├── settings_handler.py  # 设置页面处理器
│   ├── settings_service.py  # 设置业务服务
│   ├── templates.py     # 页面模板
│   └── ...
